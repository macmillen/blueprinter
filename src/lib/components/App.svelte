<script lang="ts">
  ...
  const onShapeMove = (selectedId: string, position: Vector2D) => {
    shapes = shapes.reduce<Shape[]>(
      (acc, shape) => [...acc, shape.id === selectedId ? { ...shape, ...position } : shape],
      []
    );
  };
  ...
  const onSelect = (selectedId: string | undefined) => (selectedShapeId = selectedId);
...
<div class="grid grid-cols-12">
  <div class="col-span-2 overflow-y-auto" style="max-height: calc(100vh - 72px);">
    <LeftSidebar {shapes} {selectedShape} {onChange} {onSelectShapeId} {onDeleteShape} />
  </div>
